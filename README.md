...
## ⬇️ [<ins>**DBTable_v1.4_win64.zip 내려받기 (81.7 MB - 최신 버전)**</ins>](https://github.com/wookoon2024/DBTable/raw/main/release/DBTable_v1.4_win64.zip)
...
```
adb16c986929a094f9899eb036b958bb28808d02f5b52be9c53ca8337846b153  DBTable_v1.4_win64.zip
f98a16d7c0b9c6e53724eb9ee07cdc21e490a29056dff950b2994b61b6c0c252  DB돋보기.exe
```

PowerShell 에서 확인:

```powershell
Get-FileHash .\release\DBTable_v1.4_win64.zip -Algorithm SHA256
```
...
### 1. 메인 시작 가이드 및 대시보드
![메인 시작 가이드](screenshots/01_시작가이드_대시보드.png)

### 2. 테이블 명세서 조회 및 단일/대량 SQL 쿼리 생성
![테이블 명세서 조회](screenshots/02_테이블명세서_조회.png)

### 3. 엑셀 불러오기 기반 대량 INSERT / UPDATE 쿼리 일괄 생성
> 좌측 `수정값(SET)`과 `조건(WHERE)` 컬럼 자동 분류, 우측 `{컬럼명}` 변수 템플릿 및 SQL 함수(`TO_DATE`, `NVL` 등) 자유 지정, 클립보드 붙여넣기(`Ctrl+V`) 지원
![엑셀 대량 쿼리 생성](screenshots/03_엑셀대량쿼리생성_UPDATE.png)

### 4. 전체 테이블 관계도 (ERD 다이어그램)
![전체 테이블 관계도](screenshots/04_전체테이블관계도_ERD.png)

### 5. 전체 공통코드 관리 및 매핑
![전체 공통코드 관리](screenshots/05_전체공통코드_관리.png)
