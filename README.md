...
| ♻️ 기본 정보 초기화 | 전체 데이터를 지우고 기본 샘플 데이터로 복원 |
...
앱에서 **⚙️ 설정 → ♻️ 기본 정보 초기화** 를 선택하면 됩니다.
확인 절차를 두 번 거치고, 초기화 직전 상태를 `metadata_backup_<날짜시각>.db` 로 백업한 뒤 진행합니다.
(백업 파일은 `.gitignore` 에 의해 커밋되지 않습니다.)
...
> 두 방법 모두 기존 `metadata.db` 와 `attachments/` 를 **지우고 처음부터 새로 만듭니다.**
